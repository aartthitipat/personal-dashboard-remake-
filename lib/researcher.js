...
export function getHistory() {
  ...
  return db
    .prepare(
      `SELECT id, role, content, sources, created_at FROM messages WHERE member = ? ORDER BY id ASC`
    )
    .all(MEMBER)
    .map((row) => ({ ...row, sources: row.sources ? JSON.parse(row.sources) : null }));
}

function saveMessage(role, content, sources = null) {
  const db = getDb();
  const result = db
    .prepare(`INSERT INTO messages (member, role, content, sources) VALUES (?, ?, ?, ?)`)
    .run(MEMBER, role, content, sources ? JSON.stringify(sources) : null);
...
export async function sendMessage(userText) {
  ...
  const id = saveMessage("assistant", reply, sources.length ? sources : null);
